    ...
    <Container className="py-8">
      <div ref={sectionRef}>
        ...
        <div 
          ...
          `}
        >
          <div className="bg-linear-to-r from-pink-900 to-green-900 rounded-2xl p-6 shadow-xl">
        ...
        {productsToDisplay.length > 0 ? (
                ...
                .map((product: Product, index: number) => {
                  ...
                  return (
                    <div 
                      key={product?._id} 
                      className={`
                        group relative bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md 
                    ...
                    >
                      <Link href={`/product/${product?.slug?.current}`} className="block">
                        <div className="relative aspect-square bg-linear-to-br from-pink-50 to-green-50 overflow-hidden">
                          ...
                          {!isInStock && (
                            <div className="absolute inset-0 flex items-center justify-center bg-black/60">
                              <span className="px-2 py-0.5 bg-black/80 text-white text-[10px] font-bold rounded">
                                OUT
                              </span>
                            </div>
                          )}
                      ...
                      <div className="p-2.5">
                        ...
                        <div className="mt-2">
                          <AddToCartButton 
                            product={product} 
                            className="w-full py-1.5 bg-linear-to-r from-pink-800 to-green-800 hover:from-pink-900 hover:to-green-900 text-white text-[11px] font-medium rounded-lg transition-all active:scale-95"
                            disabled={!isInStock}
                          />
                        </div>
                      </div>
            ...
            {(visibleProducts < productsToDisplay.length || visibleProducts > 9) && (
              ...
              >
                {visibleProducts < productsToDisplay.length ? (
                  <button
                    onClick={loadMore}
                    className="flex items-center gap-2 px-5 py-2 bg-pink-800 hover:bg-pink-900 text-white rounded-lg text-sm font-medium transition-all"
                  >
                    <ChevronDown size={16} />
                    Load More
                  </button>
                  ...
                  <button
                    onClick={loadLess}
                    className="flex items-center gap-2 px-5 py-2 bg-green-800 hover:bg-green-900 text-white rounded-lg text-sm font-medium transition-all"
                  >
                    <ChevronUp size={16} />
                    Show Less
                  </button>
          ...
          <div 
            className={`
              flex min-h-[50vh] flex-col items-center justify-center space-y-4 px-4 text-center
              transition-all duration-700 ease-out
              ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}
            `}
          >
            <div className="w-20 h-20 bg-linear-to-br from-pink-800 to-green-900 rounded-2xl flex items-center justify-center shadow-lg">
            ...
            <Link 
              href="/"
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-linear-to-r from-pink-800 to-green-900 hover:from-pink-900 hover:to-green-900 text-white rounded-lg text-sm font-medium transition-all"
            >
              <ShoppingBag size={16} />
              Browse Products
            </Link>
